...
@property (nonatomic, retain) MP_MPBaseInterstitialAdapter *currentAdapter;
...
- (void)presentNonNativeInterstitialForAdapter:(MP_MPBaseInterstitialAdapter *)adapter
...
+ (MP_MPInterstitialAdController *)interstitialAdControllerForAdUnitId:(NSString *)ID
{
	NSMutableArray *controllers = [MP_MPInterstitialAdController sharedInterstitialAdControllers];

	@synchronized(self)
	{
		// Find the correct ad controller based on the ad unit ID.
		MP_MPInterstitialAdController *controller = nil;
		for (MP_MPInterstitialAdController *c in controllers)
		{
		...
		if (!controller)
		{
			controller = [[[MP_MPInterstitialAdController alloc] initWithAdUnitId:ID] autorelease];
...
{
	NSMutableArray *sharedInterstitialAdControllers = 
		[MP_MPInterstitialAdController sharedInterstitialAdControllers];
...
- (id)initWithAdUnitId:(NSString *)ID
{
	if (self = [super init])
	{
		...
		_adView = [[MP_MPInterstitialAdView alloc] initWithAdUnitId:self.adUnitId size:bounds.size];
...
- (NSUInteger)supportedInterfaceOrientations
{
    NSUInteger applicationSupportedOrientations =
        [[UIApplication sharedApplication] supportedInterfaceOrientationsForWindow:MP_MPKeyWindow()];
...
- (UIInterfaceOrientation)preferredInterfaceOrientationForPresentation
{
    ...
    UIInterfaceOrientation currentInterfaceOrientation = MP_MPInterfaceOrientation();
...
- (void)adViewDidLoadAd:(MP_MPAdView *)view
{
	_ready = YES;
...
- (void)adViewDidFailToLoadAd:(MP_MPAdView *)view
{
	_ready = NO;
...
- (void)willPresentModalViewForAd:(MP_MPAdView *)view
{
...
- (void)didDismissModalViewForAd:(MP_MPAdView *)view
{
...
- (void)adView:(MP_MPAdView *)view didReceiveResponseParams:(NSDictionary *)params
{
	...
	NSString *classString = [[MP_MPAdapterMap sharedAdapterMap] classStringForAdapterType:adapterType];
	Class cls = NSClassFromString(classString);
	if (cls != nil)
	{
		[self.currentAdapter unregisterDelegate];
		self.currentAdapter = [(MP_MPBaseInterstitialAdapter *)
...
- (void)adViewShouldClose:(MP_MPAdView *)view
{
