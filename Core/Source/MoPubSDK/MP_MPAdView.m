...
- (id)initWithAdUnitId:(NSString *)adUnitId size:(CGSize)size adManager:(MP_MPAdManager *)adManager;
...
{   
    MP_MPAdManager *adManager = [[[MP_MPAdManager alloc] init] autorelease];
...
{
    if (!_adContentView || [_adContentView isKindOfClass:[MR_MRAdView class]]) return _originalSize;
...
@implementation MP_MPInterstitialAdView

@synthesize isDismissed = _isDismissed;

- (id)initWithAdUnitId:(NSString *)adUnitId size:(CGSize)size
{
    MP_MPInterstitialAdManager *adManager = [[[MP_MPInterstitialAdManager alloc] init] autorelease];
	return [self initWithAdUnitId:adUnitId size:size adManager:adManager];
}
...
- (void)setAdContentView:(UIView *)view
{
    ...
    if ([view isKindOfClass:[MR_MRAdView class]])
    {
        // Disable expansion.
    }
