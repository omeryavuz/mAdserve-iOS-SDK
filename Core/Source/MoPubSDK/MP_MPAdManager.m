...
#import "MPGlobal.h"
#import "MP_MPMraidAdapter.h"
#import "MP_MPMraidInterstitialAdapter.h"
#import "MPKeywordProvider.h"
#import "AdSdk_CJSONDeserializer.h"

NSString * const k_kTimerNotificationName = @"Autorefresh";
NSString * const k_kErrorDomain = @"mopub.com";
NSString * const k_kMoPubUrlScheme = @"mopub";
NSString * const k_kMoPubCloseHost = @"close";
NSString * const k_kMoPubFinishLoadHost = @"finishLoad";
NSString * const k_kMoPubFailLoadHost = @"failLoad";
NSString * const k_kMoPubInAppHost = @"inapp";
NSString * const k_kMoPubCustomHost = @"custom";
NSString * const k_kMoPubInterfaceOrientationPortraitId = @"p";
NSString * const k_kMoPubInterfaceOrientationLandscapeId = @"l";
const CGFloat k_kMoPubRequestTimeoutInterval = 10.0;
const CGFloat k_kMoPubRequestRetryInterval = 60.0;
...
NSString * const k_kAnimationHeaderKey = @"X-Animation";
...
NSString * const k_kAdTypeClear = @"clear";
NSString * const k_kAdTypeMraid = @"mraid";

@interface MP_MPAdManager ()
...
- (void)replaceCurrentAdapterWithAdapter:(MP_MPBaseAdapter *)newAdapter;
...
- (NSURL *)serverRequestURL {
    ...
    if (!MP_MPAdvertisingTrackingEnabled()) {
...
- (NSString *)orientationQueryStringComponent
{
	...
	NSString *orientString = UIInterfaceOrientationIsPortrait(orientation) ?
		k_kMoPubInterfaceOrientationPortraitId : k_kMoPubInterfaceOrientationLandscapeId;
	return [NSString stringWithFormat:@"&o=%@", orientString];
}

- (NSString *)scaleFactorQueryStringComponent
{
	return [NSString stringWithFormat:@"&sc=%.1f", MP_MPDeviceScaleFactor()];
}
...
- (void)adLinkClicked:(NSURL *)URL
{
    ...
    [_currentBrowserController release];
	_currentBrowserController = [[MP_MPAdBrowserController alloc] initWithURL:desiredURL 
                                                                  delegate:self];
...
- (void)connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response 
{
	...
	urlString = [headers objectForKey:k_kClickthroughHeaderKey];
	self.clickURL = urlString ? [NSURL URLWithString:urlString] : nil;

	urlString = [headers objectForKey:k_kLaunchpageHeaderKey];
	self.interceptURL = urlString ? [NSURL URLWithString:urlString] : nil;

	urlString = [headers objectForKey:k_kFailUrlHeaderKey];
	self.failURL = urlString ? [NSURL URLWithString:urlString] : nil;

	urlString = [headers objectForKey:k_kImpressionTrackerHeaderKey];
	self.impTrackerURL = urlString ? [NSURL URLWithString:urlString] : nil;

	NSString *shouldInterceptLinksString = [headers objectForKey:k_kInterceptLinksHeaderKey];
	if (shouldInterceptLinksString)
		self.shouldInterceptLinks = [shouldInterceptLinksString boolValue];

	NSString *scrollableString = [headers objectForKey:k_kScrollableHeaderKey];
	if (scrollableString)
		self.adView.scrollable = [scrollableString boolValue];

	NSString *widthString = [headers objectForKey:k_kWidthHeaderKey];
	NSString *heightString = [headers objectForKey:k_kHeightHeaderKey];
	...
	NSString *refreshString = [headers objectForKey:k_kRefreshTimeHeaderKey];
	if (refreshString)
	{
		...
		self.autorefreshTimer = [MP_MPTimer timerWithTimeInterval:interval
													  target:_timerTarget 
	...
	NSString *animationString = [headers objectForKey:k_kAnimationHeaderKey];
	if (animationString)
	...
	NSString *networkTypeHeader = [[(NSHTTPURLResponse *)response allHeaderFields] 
								   objectForKey:k_kNetworkTypeHeaderKey];
	...
	// Determine ad type.
	NSString *typeHeader = [headers	objectForKey:k_kAdTypeHeaderKey];

	if (!typeHeader || [typeHeader isEqualToString:k_kAdTypeClear]) {
		...
		[self scheduleAutorefreshTimerIfEnabled];
		return;
	} else if ([typeHeader isEqualToString:k_kAdTypeHtml] || 
			[typeHeader isEqualToString:k_kAdTypeInterstitial]) {
		// HTML ad, so just return. connectionDidFinishLoading: will take care of the rest.
		[self replaceCurrentAdapterWithAdapter:nil];
		return;
	} else if ([typeHeader isEqualToString:k_kAdTypeMraid]) {
		[self replaceCurrentAdapterWithAdapter:nil];
	...
	NSString *classString = [[MP_MPAdapterMap sharedAdapterMap] classStringForAdapterType:typeHeader];
	Class cls = NSClassFromString(classString);
	if (cls != nil)
	{
		MP_MPBaseAdapter *newAdapter = [(MP_MPBaseAdapter *)[[cls alloc] initWithAdapterDelegate:self] autorelease];
...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error
{
	...
	{
		self.autorefreshTimer = [MP_MPTimer timerWithTimeInterval:k_kMoPubRequestRetryInterval
														target:_timerTarget 
													  selector:@selector(postNotification) 
													  userInfo:nil 
													   repeats:NO];
...
- (void)handleMraidRequest
{
	MP_MPMraidAdapter *adapter = [[[MP_MPMraidAdapter alloc] initWithAdapterDelegate:self] autorelease];
...
- (void)logResponseBodyToConsole:(NSData *)data
{
	if (MP_MPLogGetLevel() <= MPLogLevelTrace)
	{
 ...
 navigationType:(UIWebViewNavigationType)navigationType
{
	...
	if ([[URL scheme] isEqualToString:k_kMoPubUrlScheme])
	{
		NSString *host = [URL host];
		if ([host isEqualToString:k_kMoPubCloseHost])
		{
			[self.adView didCloseAd:nil];
		}
		else if ([host isEqualToString:k_kMoPubFinishLoadHost])
		{
			_isLoading = NO;
		...
		else if ([host isEqualToString:k_kMoPubFailLoadHost])
		{
			_isLoading = NO;
	    ...
	    else if ([host isEqualToString:k_kMoPubInAppHost])
		{
			[self trackClick];
			NSDictionary *queryDict = [self dictionaryFromQueryString:[URL query]];
			[[MP_MPStore sharedStore] initiatePurchaseForProductIdentifier:[queryDict objectForKey:@"id"] 
												quantity:[[queryDict objectForKey:@"num"] intValue]];
		}
	    else if ([host isEqualToString:k_kMoPubCustomHost])
		{
			[self trackClick];
			NSDictionary *queryDict = [self dictionaryFromQueryString:[URL query]];
...
- (void)showLoadingIndicatorAnimated:(BOOL)animated
{
    [MP_MPProgressOverlayView presentOverlayInWindow:self.adView.window
                                         animated:animated
                                         delegate:self];
}
...
- (void)hideLoadingIndicatorAnimated:(BOOL)animated
{
    UIWindow *window = self.adView.window ? self.adView.window : MP_MPKeyWindow();
    [MP_MPProgressOverlayView dismissOverlayFromWindow:window animated:animated];
}
...
- (void)adLinkClicked:(NSURL *)URL
{
    if ([(MP_MPInterstitialAdView *)_adView isDismissed]) {
