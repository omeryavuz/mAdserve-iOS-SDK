...
#import "MR_MRAdViewDisplayController.h"
...
@interface MR_MRAdViewDisplayController ()

@property (nonatomic, retain) MR_MRAdView *twoPartExpansionView;
...
@implementation MR_MRAdViewDisplayController
@synthesize view = _view;
@synthesize currentState = _currentState;
@synthesize twoPartExpansionView = _twoPartExpansionView;

- (id)initWithAdView:(MR_MRAdView *)adView allowsExpansion:(BOOL)allowsExpansion 
    closeButtonStyle:(MRAdViewCloseButtonStyle)closeButtonStyle
{
    ...
    if (self) {
        ...
        _viewabilityTimerTarget = [[MP_MPTimerTarget alloc] 
                                   initWithNotificationName:kViewabilityTimerNotificationName];
        _viewabilityTimer = [[MP_MPTimer scheduledTimerWithTimeInterval:kViewabilityTimerInterval
        ...
        _dimmingView = [[MR_MRDimmingView alloc] initWithFrame:MP_MPKeyWindow().frame];
...
- (void)initializeJavascriptState {
    NSArray *properties = [NSArray arrayWithObjects:
                           [MR_MRScreenSizeProperty propertyWithSize:MP_MPApplicationFrame().size],
                           [MR_MRStateProperty propertyWithState:_currentState],
                           nil];
...
- (void)expandToFrame:(CGRect)frame withURL:(NSURL *)url blockingColor:(UIColor *)blockingColor
    ...
    [MP_MPKeyWindow() addSubview:_dimmingView];
    
    if (url) {
        self.twoPartExpansionView = [[[MR_MRAdView alloc] initWithFrame:self.view.frame 
                                                     allowsExpansion:NO
...
- (void)applyRotationTransformForCurrentOrientationOnView:(UIView *)view {
    ...
    switch (MP_MPInterfaceOrientation()) {
...
- (void)assignRandomTagToDefaultSuperview {
    ...
    } while ([MP_MPKeyWindow() viewWithTag:_parentTag]);
    
    self.view.superview.tag = _parentTag;
}

- (void)restoreDefaultSuperviewTag {
    [[MP_MPKeyWindow() viewWithTag:_parentTag] setTag:_originalTag];
}
...
- (void)moveViewFromWindowToDefaultSuperview {
    UIView *defaultSuperview = [MP_MPKeyWindow() viewWithTag:_parentTag];
...
- (void)constrainViewBoundsToApplicationFrame {
    ...
    CGRect applicationFrame = MP_MPApplicationFrame();
...
- (void)expandAnimationDidStop {
    ...
    [_view fireChangeEventForProperty:[MR_MRStateProperty propertyWithState:_currentState]];
