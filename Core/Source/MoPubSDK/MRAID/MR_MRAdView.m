...
@interface MR_MRAdView ()

@property (nonatomic, retain) NSMutableData *data;
@property (nonatomic, retain) MR_MRAdViewBrowsingController *browsingController;
@property (nonatomic, retain) MR_MRAdViewDisplayController *displayController;
   ...
   closeButtonStyle:(MRAdViewCloseButtonStyle)style placementType:(MRAdViewPlacementType)type
{
    ...
    if (self) {
        ...
        _browsingController = [[MR_MRAdViewBrowsingController alloc] initWithAdView:self];
        _displayController = [[MR_MRAdViewDisplayController alloc] initWithAdView:self
...
- (BOOL)isViewable {
    return MP_MPViewIsVisible(self);
}
...
- (void)initializeJavascriptState {
    ...
    [self fireChangeEventForProperty:[MR_MRPlacementTypeProperty propertyWithType:_placementType]];
...
- (BOOL)tryProcessingURLStringAsCommand:(NSString *)urlString {
    ...
    NSDictionary *parameters = MP_MPDictionaryFromQueryString(parameterString);
    
    return [self tryProcessingCommand:commandType parameters:parameters];
}

- (BOOL)tryProcessingCommand:(NSString *)command parameters:(NSDictionary *)parameters {
    MR_MRCommand *cmd = [MR_MRCommand commandForString:command];
...
- (void)adViewableDidChange:(BOOL)viewable {
    [self fireChangeEventForProperty:[MR_MRViewableProperty propertyWithViewable:viewable]];
}
